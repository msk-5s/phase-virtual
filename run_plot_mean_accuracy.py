...
def main(): # pylint: disable=too-many-locals, too-many-statements
    ...
    for day in range(1, 8):
        for dataset in results.keys():
            ...
            results[dataset].append(
                frame[f"accuracy_{model_name}_{dataset}_{noise_name}"].mean()
            )

            results_v[dataset].append(
                frame[f"accuracy_{model_name}_{dataset}_v_{noise_name}"].mean()
            )
    ...
    yticks = [round(x, 2) for x in np.arange(start=0.30, stop=1.05, step=0.1)]
    axs.set_yticks(yticks)
    axs.set_ylim([yticks[0], yticks[-1] + 0.10])
    axs.set_xticks(days)
